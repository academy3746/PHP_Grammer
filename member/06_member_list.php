...
# `band` 테이블에 등록되어있는 `mb_id` 값을 계수할 것
$sql = "select count(*) as 'cnt' from metalhero.band";
$result = mysqli_query($conn, $sql);
$row = mysqli_fetch_assoc($result);
$total_cnt = $row['cnt'];

# `band` 테이블에 등록되어 있는 데이터 목록들을 일괄 조회할 것
$sql = "select * from metalhero.band order by mb_datetime desc";
$result = mysqli_query($conn, $sql);
...
<div class="container">
    ...
    <div class="box">
        <table class="table table-striped">
            ...
            <?php
            if ($total_cnt === 0) {
                echo "<tr><td colspan='8'>등록된 회원이 존재하지 않습니다.</td></tr>";
            }
            ?>
            </tbody>
        </table>
